...
public class AdminRegistrationController {
    ...
    public AdminRegistrationController(App mainApp) {
    ...
    private void createView() {
        ...
        mainView.add(firstNameLabel, 2, 3);
        mainView.add(firstNameField, 3, 3);

        Label dobLabel = new Label("Date de naissance:");
        dobField = new DatePicker();
        mainView.add(dobLabel, 0, 4);
        mainView.add(dobField, 1, 4);
        ...
        Label employeeCodeLabel = new Label("Code employé:");
        employeeCodeField = new TextField();
        mainView.add(employeeCodeLabel, 0, 5);
        mainView.add(employeeCodeField, 1, 5);

        Label cinNumberLabel = new Label("Numéro CIN:");
        cinNumberField = new TextField();
        mainView.add(cinNumberLabel, 2, 5);
        mainView.add(cinNumberField, 3, 5);

        // Infos de connexion
        Label loginInfoLabel = new Label("infos de connexion");
        mainView.add(loginInfoLabel, 4, 2, 2, 1);

        Label usernameLabel = new Label("Choisissez un nom d'utilisateur:");
        usernameField = new TextField();
        mainView.add(usernameLabel, 4, 3);
        mainView.add(usernameField, 5, 3);

        Label passwordLabel = new Label("Mot de passe:");
        passwordField = new PasswordField();
        mainView.add(passwordLabel, 4, 4);
        mainView.add(passwordField, 5, 4);

        Label confirmPasswordLabel = new Label("Confirmer le mot de passe:");
        confirmPasswordField = new PasswordField();
        mainView.add(confirmPasswordLabel, 4, 5);
        mainView.add(confirmPasswordField, 5, 5);

        // Bouton S'inscrire
        Button signUpButton = new Button("S'inscrire");
        signUpButton.setOnAction(event -> handleSignUp());
        mainView.add(signUpButton, 1, 6);

        // Besoin d'assistance?
        Label assistanceLabel = new Label("Besoin d'assistance?");
        mainView.add(assistanceLabel, 5, 7);
        ...
        mainView.add(contactButton, 5, 8);
    }
    ...
    private void handleSignUp() {
        ...
        String cinNumber = cinNumberField.getText();
        ...
        // Logique pour inscrire un nouvel administrateur dans la base de données
        if (!"".equals(lastName) && !"".equals(firstName) && !"".equals(dob) && !"".equals(phone) && !"".equals(username) && !"".equals(password) && !"".equals(confirmPassword) && !"".equals(employeeCode) && !"".equals(cinNumber)) { 
            try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/agenceavion", "root", "")) {
                String query = "INSERT INTO admin (nom, prenom, date_naissance, telephone, code_employe, cin, nom_utilisateur, mot_de_passe) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement statement = connection.prepareStatement(query);
                statement.setString(1, lastName);
                statement.setString(2, firstName);
                statement.setString(3, dob);
                statement.setString(4, phone);
                statement.setString(5, employeeCode);
                statement.setString(6, cinNumber);
                statement.setString(7, username);
                statement.setString(8, password);
                statement.executeUpdate();
                mainApp.showAdminLoginView();
